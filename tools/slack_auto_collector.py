...
用法：
  python3 slack_auto_collector.py --name "张三" --output-dir ./collected_materials/zhangsan
...
CONFIG_PATH = Path.home() / ".ccjincc-skill-tools" / "slack_config.json"
...
示例：
  ...
  python3 slack_auto_collector.py --name "john.doe" --output-dir ./collected_materials/john --msg-limit 500
        """,
    )
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help="输出目录（默认 ./collected_materials/{name}）",
    )
    parser.add_argument(
        "--msg-limit",
        type=int,
    ...
    output_dir = (
        ...
        if args.output_dir
        else Path(f"./collected_materials/{args.name}")
    )
