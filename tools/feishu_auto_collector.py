...
用法：
  # 群聊采集（原有方式）
  python3 feishu_auto_collector.py --name "张三" --output-dir ./collected_materials/zhangsan
...
CONFIG_PATH = Path.home() / ".ccjincc-skill-tools" / "feishu_config.json"
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default=None, help="输出目录（默认 ./collected_materials/{name}）")
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"./collected_materials/{args.name or 'target'}")
