...
用法：
  python3 dingtalk_auto_collector.py --name "张三" --output-dir ./collected_materials/zhangsan
...
CONFIG_PATH = Path.home() / ".ccjincc-skill-tools" / "dingtalk_config.json"
...
def main() -> None:
    ...
    config = load_config()
    output_dir = Path(args.output_dir) if args.output_dir else Path(f"./collected_materials/{args.name}")
